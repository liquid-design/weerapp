...
| Eerste geslaagde build | `fetch_boundaries.py it` → IT meegeleverd, 187 zones, manifest `ok` · `fetch_warning_status.py it` → live-bulletin faalde bij eerste run (zie UNKNOWN — **oorzaak later gecorrigeerd: fetcher-bug, niet een dode feed**), bestaande `warning_status.json` behouden |
...
| Bekende UNKNOWN-gevallen | (1) **IT live-kleuren** — bij eerste run `sources.IT.ok=false`. *Correctie na de deploy:* de feed is niet dood; de diagnose "bevroren sinds 2022" was een artefact van de GitHub contents-API (cap 1000). De echte oorzaak was een fetcher-bug (vaste HHMM-gok); sinds de fix (git-tree-discovery) werken de IT-kleuren, `sources.IT.ok=true`. (2) **DE/NL/AT** geometrie niet lokaal bij mijlpaal 1 — later uitgerold + als shipped geometrie in git gezet. (3) **SI** heeft per ontwerp geen officieel zonebestand (ARSO-regio's, MeteoAlarm-token vereist) — geldige UNKNOWN |
